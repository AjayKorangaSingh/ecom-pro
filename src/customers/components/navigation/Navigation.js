...
export default function Navigation() {
  ...
  const handle = (event, category, section, item) => {
    event.stopPropagation(); // Prevents further propagation of the event
    setOpen(false); // Close the dialog

    // Use setTimeout to navigate after the dialog closes
    setTimeout(() => {
      const newPath = `/${category.id}/${section.id}/${item.name
        .replace(/\s+/g, "-")
        .toLowerCase()}`;
      navigate(newPath);
  ...
  return (
    <div className="bg-white z-40">
                                    ...
                                    >
                                      {section.items.map((item) => (
                                        ...
                                        >
                                          <p
                                            onClick={(e) =>
                                              handle(e, category, section, item)
                                            }
      ...
      <AuthModel handleClose={handleClose} open={openAuth} />
    </div>
  );
}
